...
C_PREFIX = 10
H = 80
# Action 2M.1: one reference plus SIXTEEN pre-outcome-selected siblings.  The
# first M0 saw 7 candidates per anchor and produced 139/144 tied held-out pairs;
# more siblings per anchor is the registered lever on ranking support, and the
# selection scheme is unchanged (farthest-point diversity, then disjoint
# matched-random), only widened 3+3 -> 8+8.
N_ALT_DRAWS = 32
N_DIVERSITY = 8
N_RANDOM = 8
CANDIDATES = 1 + N_DIVERSITY + N_RANDOM                            # 17

# Fresh, source-disjoint from the v082 bank (which consumed 3500-3575).
QUEUES = {
    "train": tuple(range(3600, 3670)),
    "val": tuple(range(3670, 3688)),
...
def select_wide(alt_env, unique_idx, root: str, anchor_id: str) -> dict:
    """Farthest-point diversity then disjoint matched-random, at 8+8.

    Same scheme as `lcwm.v081p_exec.select_candidates`, which is hard-wired to
    3+3: standardized action features, a presealed hash-derived start, and a
    deterministic index tie-break.  Nothing here consults an outcome.
    """
    idx = list(unique_idx)
    X = np.stack([np.asarray(alt_env[i]).reshape(-1) for i in idx])
    mu, sd = X.mean(0), X.std(0)
    Z = (X - mu) / np.where(sd > 1e-8, sd, 1.0)
    start = int(np.frombuffer(hashlib.sha256(
        f"{root}|fps|{anchor_id}".encode()).digest()[:4], dtype=np.uint32)[0] % len(idx))
    chosen = [start]
    ...
    rng = np.random.default_rng(int(np.frombuffer(hashlib.sha256(
        f"{root}|rand|{anchor_id}".encode()).digest()[:4], dtype=np.uint32)[0]))
    rand = [int(i) for i in rng.choice(rest, size=N_RANDOM, replace=False)]
    return {"diversity": div, "random": rand, "fps_start": start, "n_pool": len(idx)}
...
def build_training_pool(runner, env, anchor, root: str) -> dict:
    ...
    selection = select_wide(alt_env, unique_idx, root, anchor.anchor_id)
    indices = selection["diversity"] + selection["random"]
    ids = (["reference"] + [f"div{i}" for i in range(N_DIVERSITY)]
    ...
    return {
        ...
        "is_reference": torch.tensor(
            [True] + [False] * (CANDIDATES - 1), dtype=torch.bool
        ),
