#!/usr/bin/env python
"""V7.4B — build one recovery-crossed training tranche (plan_and_progress/archive/daily/2026-08-09.md).

Frozen budget (registered before collection):
  seeds per task k (fresh 2500 family, disjoint from every prior
  family and all behavior panels):
    ...
    teacher source 2505+10k                        (2 anchors)
  = 30 train + 10 dev acquisition anchors, 10 outcome-blind teacher
  anchors.

Blocker-targeted anchors (V7.4B retargeting): candidate decisions
where the canonical first_unresolved matches the task's EARLY family
(the registered V7.2D terminal-blocker milestones) or LATE family
(placement/close prefixes among the last two canonical ordered
subgoals) AND the reachability rule holds. Train sources take one
EARLY + one LATE anchor via a joint gap-compatible pair search
(family_shortfall recorded, never silently substituted); dev and
teacher use the inherited diversity-first fallback over the widened
pool. >=5 decisions apart, state-type diversity preferred.
...
the V7.4D ledgers are sealed.

Output root: results/<DATE>_v074_data_r1/ per the required artifact
...
# EARLY family: registered V7.2D terminal-blocker milestones per
# task (first-pick blockers); a candidate anchor's canonical
# first_unresolved must start with one of these prefixes to match
# the EARLY family
EARLY_BLOCKERS = {
...
# LATE family: the PLACEMENT/CLOSE-REGION prefixes among the last
# two ordered_subgoals of each task's canonical GoalSpec, read from
# results/libero_loho_public_v1/goal_spec_manifest_v067.json
# (t1_canonical..t5_canonical) and hardcoded as a frozen literal —
# the registration ("mid/late-chain decision (placement or
# close-region)") freezes targeting before execution; pick_up
# subgoals in the same window are deliberately excluded per that
# parenthetical; never derived at runtime
LATE_BLOCKERS = {
    "loho_t1_drawer": ["place chocolate_pudding_1",
                       "close wooden_cabinet_1_top_region"],
    "loho_t2_basket3": ["place tomato_sauce_1"],
    "loho_t3_tray": ["place butter_1"],
    "loho_t4_tray": ["place chocolate_pudding_1"],
    "loho_t5_drawer_cabinet": ["place akita_black_bowl_1"],
}
...
REC_TERM_MAX = 300
REREACH_ATOL = 2e-3
ROLES = [("train", 2500, 2, "acq"), ("train", 2501, 2, "acq"),
         ("train", 2502, 2, "acq"), ("dev", 2503, 1, "acq"),
         ("dev", 2504, 1, "acq"), ("teacher", 2505, 2, "teach")]
...
def main() -> None:
    ...
    if not mp.exists():
        mp.write_text(json.dumps({
            "schema": "v074_data_manifest_v1", "run_schema": "v074",
            "run_id": RID, "git_sha": git_sha,
            "seed_roles": ROLES,
            "early_blockers": EARLY_BLOCKERS,
            "late_blockers": LATE_BLOCKERS,
            "alt_goals": ALT_GOAL,
            ...
            "panels_excluded": "{1700..1740} sealed, {1750..1790} "
                               "consumed, {1800..1840} consumed, "
                               "{1850..1890} reserved dev",
        }, indent=2))
    ...
    if not anchor_path.exists():
            ...
            for role, base, n_anchor, kind in ROLES:
                ...
                def gap_ok(c):
                    return all(abs(c["decision"] - x["decision"])
                               >= GAP for x in chosen)
                # V7.4B family-targeted passes: TRAIN sources only
                # (the registration targets train-source anchors;
                # dev/teacher keep the inherited diversity-first
                # fallback over the widened candidate pool). A
                # gap-compatible (early, late) PAIR is searched
                # jointly before any greedy pick so a low-decision
                # greedy choice cannot gap-block a feasible pair
                # (review finding); family_shortfall is recorded
                # only when no compatible assignment exists.
                if role == "train":
                    earls = [c for c in src["anchor_candidates"]
                             if c["family"] == "early"]
                    lates = [c for c in src["anchor_candidates"]
                             if c["family"] == "late"]
                    pairs = [(e, lt) for e in earls for lt in lates
                             if abs(e["decision"] - lt["decision"])
                             >= GAP]
                    pair = next(
                        (p for p in pairs
                         if p[0]["state_type"]
                    ...
                    if pair is not None:
                        ...
                        for fam, pool in (("early", earls),
                                          ("late", lates)):
                            pick = next(
                                (c for c in pool
                                 if c not in chosen and gap_ok(c)),
                                None)
                            if pick is not None:
                                chosen.append(pick)
                                seen.add(pick["state_type"])
                            else:
                                all_anchors.append({
                                    "source_id": sid, "task": task,
                                    "seed": seed, "role": role,
                                    "kind": "family_shortfall",
                                    "family": fam,
                                    "candidates_in_family":
                                        len(pool)})
                                print(
                                    f"[anchors] family_shortfall "
                                    f"{sid}: {fam} ({len(pool)} "
                                    f"in-family cands)",
                                    flush=True)
        ...
        # registered per-TASK targeting verdict: >= 1 train anchor
        # in each family (review finding: per-source shortfall rows
        # alone left the task-level condition implicit)
        fam_cov = {t: {"early": 0, "late": 0} for t in TASK_ORDER}
        for a in all_anchors:
            if a.get("kind") == "acq" and a["role"] == "train":
                fam_cov[a["task"]][a["family"]] += 1
        task_family_coverage = {
            t: {**fam_cov[t],
                "registered_ok": fam_cov[t]["early"] >= 1
                and fam_cov[t]["late"] >= 1}
            for t in TASK_ORDER}
        print(f"[anchors] task_family_coverage: "
              f"{json.dumps(task_family_coverage)}", flush=True)
        anchor_path.write_text(json.dumps({
            "schema": "v074_anchor_manifest_v1",
            "note": "early/late blocker-family targeted; frozen "
                    "from pre-action stock state before any "
                    "candidate generation; SHORTFALL and "
                    "family_shortfall rows record under-filled "
                    "budgets explicitly",
        ...
        if n_fshort:
            by_role["family_shortfall_rows"] = n_fshort
    ...
    for a in anchors:
        ...
        try:
            ...
            shard = {
                "schema": "v074_shard_v1", "run_schema": "v074",
    ...
    for a in anchors:
        ...
        torch.save({"schema": "v074_teacher_bank_v1",
                    "anchor": akey, "task": task,
                    "source_id": sid, "decision": d,
                    "bank": bank,
                    "note": "UNEXECUTED; outcomes hidden until "
                            "V7.4D ledgers sealed"}, tb_path)
    ...
    for p_ in sorted((root / "shards").glob("*.pt")):
        ...
        for tr in policy_trs:
            ...
            if tr["recovery_terminal"] is not None:
                ...
                n_recterm += 1
            # the registered V7.4B cell: a milestone-positive
            # RECOVERY branch that also carries a
            # recovery-to-terminal continuation attempt (review
            # finding: counting any milestone-positive policy
            # branch let the named missing cell pass unfilled)
            if (tr["branch_key"] in ("rec_canon", "rec_alt")
                    and per_subgoal_flip(tr, canon)
                    and tr["recovery_terminal"] is not None):
                sup[cell]["milestone_pos_recovery_terminal"] += 1
    ...
    support = {
        ...
        "registered_cell_checks": {
            c: {"pos_nonpos_at_2train_sources_or_1dev":
                ...
                "milestone_pos_recovery_terminal_present":
                    sup[c]["milestone_pos_recovery_terminal"] >= 1}
            for c in ALL_CELLS},
        # registered per-TASK verdict (train support is what feeds
        # V7.4C/D): every task must produce >= 1 milestone-positive
        # recovery branch with a recovery-to-terminal attempt
        "registered_task_checks": {
            tk: {
                "milestone_pos_recovery_terminal_train":
                    sup[f"{tk}|train"]
                    ["milestone_pos_recovery_terminal"] >= 1}
