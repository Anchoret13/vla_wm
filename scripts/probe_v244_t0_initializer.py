...
TEMPORAL_REPROMPT_MIN_DISPLACEMENT_PX = 12.0
TEMPORAL_EVIDENCE_GRACE_FRAMES = 1
SIFT_TEMPLATE_DERIVATION = {
    ...
    "data_boundary": (
        "RGB-only calibration; no event, success, simulator-state, "
        "or task-definition input"
    ),
}
...
def make_dino_points(
    ...
    *,
    ...
    for feature_index, episode_id in enumerate(episode_ids, start=len(calibration_ids)):
        ...
        for name, prototype in prototypes.items():
            ...
            if clustered:
                ...
                gy, gx = ya + dy, xa + dx
                score_record = {
                    "max_patch_cosine": float(similarity.flatten()[flat_index])
                }
    ...
    provenance = {
        ...
        "prototype_rule": (
            "mean normalized patch embedding in 3x3 neighborhoods around "
            "frozen calibration points"
        ),
        "search_regions_xyxy": (
...
def make_sift_points(
    ...
    provenance = {
        ...
        "location_rule": (
            "inverse-descriptor-distance weighted centroid of largest "
            "connected cluster"
        ),
...
class CausalSiftAffineLocator:
    ...
    def __init__(self, calibration: Mapping[int, list[dict[str, Any]]]) -> None:
        ...
        self.provenance = {
            "family": (
                "OpenCV SIFT cluster + affine-partial-2D RANSAC causal "
                "re-acquisition"
            ),
            "opencv_version": cv2.__version__,
            "offline": True,
            "causal_rule": (
                "each frame uses only that frame RGB plus the fixed "
                "episode-0 template"
            ),
            "later_locator_failure": (
                "soft: omit that object prompt and use past-only SAM2 propagation"
            ),
            "calibration_episode_id": 0,
            ...
            "prompt_rule": (
                "inverse-distance weighted centroid of RANSAC-inlier "
                "scene keypoints"
            ),
            "relation_evidence_rule": (
                "use affine-projected template center only when it lies in "
                "the current prompted target mask; "
...
def track_episode(
    ...
    *,
    ...
    temporal_evidence_grace_flips = {name: 0 for name in TARGET_OBJECTS}
    temporal_last_sift_sam_frame: dict[str, int | None] = {
        name: None for name in TARGET_OBJECTS
    }
    temporal_last_sift_sam_mask_centroid: dict[
        str, tuple[float, float] | None
    ] = {name: None for name in TARGET_OBJECTS}
    temporal_last_sift_sam_bbox_diagonal: dict[str, float | None] = {
        name: None for name in TARGET_OBJECTS
    }
    temporal_last_prompt_point: dict[str, tuple[float, float] | None] = {
        name: None for name in ID_TO_NAME.values()
    }
    temporal_locator_was_valid = {name: False for name in ID_TO_NAME.values()}
    temporal_prompt_counts = {name: 0 for name in ID_TO_NAME.values()}

    def causal_outputs() -> Iterable[
        tuple[
            Any,
        ...
        for frame_index, frame in enumerate(frames):
            ...
            prompt_points: dict[str, tuple[float, float]] = {}
            for name, point in located_points.items():
                previous_prompt = temporal_last_prompt_point[name]
                should_prompt = bool(
                    frame_index == 0
                    or not temporal_locator_was_valid[name]
                    or previous_prompt is None
                    or math.dist(point, previous_prompt)
                    >= TEMPORAL_REPROMPT_MIN_DISPLACEMENT_PX
                )
                if should_prompt:
                    prompt_points[name] = point
                    temporal_last_prompt_point[name] = point
                    temporal_prompt_counts[name] += 1
            for name in ID_TO_NAME.values():
                temporal_locator_was_valid[name] = name in located_points
            prompt_ids = [
                obj_id
                for obj_id in object_ids
                if ID_TO_NAME[obj_id] in prompt_points
            ]
            if prompt_ids:
                processor.add_inputs_to_inference_session(
                    inference_session=session,
                    ...
                    input_points=[
                        [
                            [list(prompt_points[ID_TO_NAME[obj_id]])]
    ...
    with torch.inference_mode():
        if causal_locator is not None:
            ...
            outputs = (
                (output, dict(points), dict(points), dict(point_scores or {}), 0)
                for output in raw_outputs
            )
        for (
            output,
            current_points,
            current_prompt_points,
            ...
            if causal_locator is not None:
                ...
                for name in TARGET_OBJECTS:
                    ...
                    sift_sam_agree = bool(
                        ...
                        == "sift_affine_projected_template_center"
                        and not any(
                            reason.startswith("target_mask_collision_with:")
                            for reason in diagnostics[name]["uncertain_reasons"]
                        )
                        and not diagnostics[name]["uncertain_reasons"]
                        and diagnostics["basket_1"]["reliable"]
                    )
                    previous_sift_sam_frame = temporal_last_sift_sam_frame[name]
                    previous_verified_centroid = (
                        temporal_last_sift_sam_mask_centroid[name]
                    )
                    previous_verified_bbox_diagonal = (
                        temporal_last_sift_sam_bbox_diagonal[name]
                    )
                    evidence_age_frames = (
                        frame_index - previous_sift_sam_frame
                        if previous_sift_sam_frame is not None
                        else None
                    )
                    current_centroid = diagnostics[name]["mask_centroid_xy"]
                    grace_displacement_px = (
                        math.dist(previous_verified_centroid, current_centroid)
                        if previous_verified_centroid is not None
                        and current_centroid is not None
                        else None
                    )
                    grace_spatial_continuity = bool(
                        grace_displacement_px is not None
                        and previous_verified_bbox_diagonal is not None
                        and grace_displacement_px <= previous_verified_bbox_diagonal
                    )
                    if sift_sam_agree:
                        temporal_last_sift_sam_frame[name] = frame_index
                        evidence_age_frames = 0
                        require(current_centroid is not None, "reliable target has no centroid")
                        temporal_last_sift_sam_mask_centroid[name] = tuple(
                            float(value) for value in current_centroid
                        )
                        bbox = diagnostics[name]["mask_bbox_xyxy"]
                        require(bbox is not None, "reliable target has no bbox")
                        temporal_last_sift_sam_bbox_diagonal[name] = math.hypot(
                            float(bbox[2] - bbox[0]), float(bbox[3] - bbox[1])
                        )
                    grace_available = bool(
                        not sift_sam_agree
                        and evidence_age_frames is not None
                        and evidence_age_frames <= TEMPORAL_EVIDENCE_GRACE_FRAMES
                        and diagnostics["basket_1"]["reliable"]
                        and grace_spatial_continuity
                    )
                    ...
                    if candidate_label in {"inside", "outside"}:
                        if candidate_label == previous_state:
                            ...
                            decision_source = (
                                "current_sift_sam"
                                if sift_sam_agree
                                else "sam_only_matches_evidence_state"
                            )
                        elif sift_sam_agree:
                            temporal_evidence_state[name] = candidate_label
                            temporal_evidence_confirmed_flips[name] += 1
                            decision = "sift_sam_confirmed_flip"
                            decision_source = "current_sift_sam"
                        elif grace_available:
                            temporal_evidence_state[name] = candidate_label
                            temporal_evidence_confirmed_flips[name] += 1
                            temporal_evidence_grace_flips[name] += 1
                            decision = "recent_sift_sam_grace_confirmed_flip"
                            decision_source = "previous_sample_sift_sam"
                        else:
                            ...
                            decision_source = "insufficient_recent_evidence"
                    ...
                    diagnostics[name]["evidence_gate_source"] = decision_source
                    diagnostics[name]["last_sift_sam_reliable_frame_before"] = (
                        previous_sift_sam_frame
                    )
                    diagnostics[name]["last_sift_sam_age_frames"] = (
                        evidence_age_frames
                    )
                    diagnostics[name]["current_sift_sam_reliable"] = sift_sam_agree
                    diagnostics[name]["grace_available"] = grace_available
                    diagnostics[name]["grace_displacement_px"] = (
                        grace_displacement_px
                    )
                    diagnostics[name]["grace_max_displacement_px"] = (
                        previous_verified_bbox_diagonal
                    )
                    diagnostics[name]["grace_spatial_continuity"] = (
                        grace_spatial_continuity
                    )
            if frame_index == 0:
            ...
            output_rows.append(
                {
                    ...
                    "prompt_frame_index": (
                        frame_index
                        if causal_locator is not None and current_prompt_points
                        else (None if causal_locator is not None else 0)
                    ),
                    "post_t0_prompt_count": prompt_count,
                    "prompt_points_xy": {
                        name: list(value)
                        for name, value in current_prompt_points.items()
                    },
                    "current_locator_points_xy": (
                        {
                            name: list(value)
                            for name, value in current_points.items()
                        }
                        if causal_locator is not None
                        else None
                    ),
        ...
        "causal_reacquisition": (
            {
                ...
                "prompt_applications_including_t0_per_object": temporal_prompt_counts,
                "reprompt_rule": (
                    "prompt at t0, after locator recovery, or when distance from that object's "
                    f"last prompt is >= {TEMPORAL_REPROMPT_MIN_DISPLACEMENT_PX}px"
                ),
                ...
                "evidence_gate_rule": (
                    "SAM-only may maintain the last evidence-backed state; "
                    "a change in either direction requires current-frame "
                    "SIFT+SAM agreement or at most one-frame-old collision-free "
                    "SIFT+SAM identity evidence with a currently reliable basket "
                    "and mask-centroid displacement no larger than the last "
                    "verified mask-bbox diagonal, otherwise raw label is uncertain"
                ),
                "evidence_confirmed_flips": temporal_evidence_confirmed_flips,
                "evidence_grace_confirmed_flips": temporal_evidence_grace_flips,
            ...
            "causal_reacquisition": (
                {
                    ...
                    "ransac_reprojection_threshold_px": (
                        SIFT_RANSAC_REPROJECTION_THRESHOLD_PX
                    ),
                    ...
                    "target_prompt_collision_distance_px_lte": (
                        TEMPORAL_TARGET_PROMPT_COLLISION_DISTANCE_PX
                    ),
                    "target_mask_collision_iou_gte": (
                        TEMPORAL_TARGET_MASK_COLLISION_IOU_GTE
                    ),
                    "reprompt_min_displacement_px": (
                        TEMPORAL_REPROMPT_MIN_DISPLACEMENT_PX
                    ),
                    "reprompt_after_locator_recovery": True,
                    "evidence_grace_frames": TEMPORAL_EVIDENCE_GRACE_FRAMES,
                    "evidence_grace_spatial_continuity": (
                        "current target-mask centroid displacement from the last verified centroid "
                        "must be <= the last verified target-mask bbox diagonal"
                    ),
                    "later_locator_failure": (
                        "omit object prompt; SAM2 uses past memory"
                    ),
                    "evidence_state_gate": (
                        "state starts outside; SAM-only may maintain but cannot change state; "
                        "a bidirectional change requires current-frame SIFT+SAM agreement or "
                        "collision-free SIFT+SAM evidence from the immediately "
                        "previous sampled frame "
                        "with a currently reliable basket and scale-adaptive spatial continuity; "
