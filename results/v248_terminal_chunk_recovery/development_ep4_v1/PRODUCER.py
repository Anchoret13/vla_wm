...
def load_inputs(
    ...
    sigma_choices = [float(np.float32(value)) for value in raw_sigma_choices]
    ...
    for episode_id in sorted(records):
        ...
        draws = [
            sigma_choices[int(sigma_rng.integers(len(sigma_choices)))]
        ...
        require(observed_sigmas == draws[: len(tape_rows)], f"episode {episode_id} sigma RNG drift")
...
def replay_episode(
    ...
    for local_index, row_index in enumerate(row_indices):
        ...
        sigma = float(tape["sigma"][row_index])
