...
two consecutive occlusion signals inside a short, fixed confirmation window.
"""
...
SCHEMA = "v247_causal_cream_state_v1"
...
class CreamStateConfig:
    ...
    occlusion_frames_required: int = 2
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_confirmation_frames < self.occlusion_frames_required:
            raise CreamStateError("confirmation window is too short")
        if self.occlusion_frames_required < 1:
            raise CreamStateError("occlusion_frames_required must be positive")
    ...
    def step(self, evidence: CreamFrameEvidence) -> CreamFrameState:
        ...
        if self.mode == "achieved":
            ...
            if evidence.cross_class_collision:
                ...
                if self.occlusion_streak >= self.config.occlusion_frames_required:
                    self.mode = "achieved"
                    transition_now = True
