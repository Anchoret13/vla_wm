...
class EyeCreamSift:
    def __init__(self, template_result_path: Path) -> None:
        path = template_result_path.expanduser().resolve()
        require(path.is_file(), f"missing template result: {path}")
        require(sha256_file(path) == EXPECTED_TEMPLATE_RESULT_SHA256, "template result drift")
        result = load_json(path)
        reject_semantic_fields(result, str(path))
        ...
        require(sha256_file(image_path) == EXPECTED_TEMPLATE_IMAGE_SHA256, "template image drift")
        ...
        require(bgr is not None, "template image decode failed")
    ...
    def locate(self, image_path: Path) -> dict[str, Any]:
        bgr = cv2.imread(str(image_path), cv2.IMREAD_COLOR)
        require(bgr is not None, f"image decode failed: {image_path}")
        enlarged = cv2.resize(
            bgr,
            None,
            fx=SIFT_UPSCALE,
    ...
    for frame_index, (frame, sift) in enumerate(zip(episode_frames, sift_rows, strict=True)):
        ...
        if track is not None:
            ...
            overlaps = [
                ...
                for item in frame.get("instances", {}).get("can", [])
